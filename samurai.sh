...
UI_CMD_43=( "Install ufw and allow ssh port 22/80" "sudo apt-get install ufw && ufw allow ssh && ufw allow 80 && sudo ufw enable" )
...
case "$SK_CMD" in
  ...
  ui)
    if [ -z $SK_OPT ]; then
      ...
      SK_CMD_TITLE=$(eval echo \${UI_CMD_$SK_OPT[0]})
      SK_CMD_CMD=$(eval echo \${UI_CMD_$SK_OPT[1]})

      printf "\t%s" "Executing :: $SK_CMD_TITLE :: $SK_CMD_CMD"
      eval $SK_CMD_CMD
    fi
    ;;
  *)
